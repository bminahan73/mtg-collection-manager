...
function normalizeWishlist(cards){
  return cards.reduce((items, card) => items.some(item => item.id === card.id) ? items : [...items, { ...card, quantity: Number(card.quantity) || 1 }], [])
}
...
function App(){
  ...
  async function toggleWishlist(card){
    ...
    const nextWishlist = alreadySaved ? wishlist.filter(item => item.id !== card.id) : [...wishlist, wishlistCard]
  ...
  async function add(card, quantity = 1){
    const existing = collection.find(item => item.id === card.id)
    const nextCollection = existing
      ? collection.map(item => item.id === card.id ? { ...item, quantity: item.quantity + quantity } : item)
    ...
    try {
      const response = await fetch(`${API_BASE}/api/collection${existing ? `/${card.id}` : ''}`, {
        ...
        body: JSON.stringify(existing ? { quantity: existing.quantity + quantity } : { card, quantity })
      })
  ...
  async function saveDeck(nextDeck){
    setDecks(previous => previous.some(deck => deck.id === nextDeck.id) ? previous.map(deck => deck.id === nextDeck.id ? nextDeck : deck) : [...previous, nextDeck])
    localStorage.setItem('mtg-decks', JSON.stringify(decks.some(deck => deck.id === nextDeck.id) ? decks.map(deck => deck.id === nextDeck.id ? nextDeck : deck) : [...decks, nextDeck]))
    const exists = decks.some(deck => deck.id === nextDeck.id)
    await fetch(`${API_BASE}/api/decks${exists ? `/${nextDeck.id}` : ''}`, { method: exists ? 'PUT' : 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ deck: nextDeck }) })
  }

  async function createDeck(event){
    event.preventDefault()
    const name = deckName.trim() || 'Untitled deck'
    const deck = { id: crypto.randomUUID(), name, format: deckFormat, commander: null, cards: [] }
    await saveDeck(deck)
  ...
  async function addToDeck(card){
    ...
    const cards = deck.cards.some(item => item.card.id === card.id)
      ? deck.cards.map(item => item.card.id === card.id ? { ...item, quantity: item.quantity + 1 } : item)
...
while(!batchSucceeded && retryCount <= 10){
    ...
    for(const item of importedCards) {
      await fetch(`${API_BASE}/api/collection`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ card: item, quantity: item.quantity }) })
      imported += item.quantity
    }
    setImportState(`Import complete: ${imported} cards added, ${skipped} rows skipped.`)
    event.target.value = ''
  }

  async function loadAssistant(){
    const deck = decks.find(item => item.id === activeDeckId)
    const commander = deck?.commander || deck?.cards?.find(item => item.card.type_line?.includes('Legendary Creature'))?.card
    if(!commander) { setAssistantResults([]); return }
    try {
      const slug = commander.name.toLowerCase().replace(/[^a-z0-9]+/g, '-')
      const edhrecResponse = await fetch(`https://json.edhrec.com/pages/commanders/${slug}.json`)
      if(edhrecResponse.ok) {
        const edhrec = await edhrecResponse.json()
        const views = (edhrec.container?.json_dict?.cardlists || []).flatMap(list => list.cardviews || []).slice(0, 8)
        const cards = await Promise.all(views.map(async view => {
          const response = await fetch(`https://api.scryfall.com/cards/named?exact=${encodeURIComponent(view.name)}`)
      ...
      const response = await fetch(`https://api.scryfall.com/cards/search?q=${encodeURIComponent(`related:${commander.id}`)}`)
      const json = await response.json()
      setAssistantResults((json.data || []).slice(0, 8))
      setAssistantSource('Scryfall related cards')
    } catch { setAssistantResults([]); setAssistantSource('') }
  }
  ...
  function openEditor(card, source){
    setEditing({ source, card: { ...card, quantity: Number(card.quantity) || 1, set_name: card.set_name || card.set?.toUpperCase() || '', rarity: card.rarity || '', condition: card.condition || 'Near Mint', language: card.language || 'English', foil: Boolean(card.foil), notes: card.notes || '' } })
  }
  ...
  async function saveEdit(event){
    ...
    if(editing.source === 'collection') updateCollection(collection.map(item => item.id === card.id ? card : item))
    else updateWishlist(wishlist.map(item => item.id === card.id ? card : item))
    try {
      const response = await fetch(`${API_BASE}/api/${editing.source}/${card.id}`, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ quantity, card }) })
      if(!response.ok) throw new Error('The card details could not be saved.')
      setEditing(null)
      setCollectionError('')
    } catch(error) {
      setCollectionError(`${error.message} Your local changes are still visible.`)
    }
  }
  ...
  useEffect(() => {
    const localDecks = (() => { try { return JSON.parse(localStorage.getItem('mtg-decks') || '[]') } catch { return [] } })()
    fetch(`${API_BASE}/api/decks`).then(response => response.ok ? response.json() : Promise.reject()).then(savedDecks => {
      setDecks(savedDecks)
  ...
  const deckPrice = activeDeck?.cards?.reduce((total, item) => total + ((cardPrice(item.card) || 0) * item.quantity), 0) || 0
  ...
  return <div className="app">
    ...
    <main>
      ...
      <section className={`wishlist drop-zone ${dragTarget === 'wishlist' ? 'drag-target' : ''}`} onDragOver={event => allowDrop(event, 'wishlist')} onDragLeave={() => setDragTarget('')} onDrop={event => dropCard(event, 'wishlist')}><div className="section-heading"><div><p className="eyebrow">WANTED LIST</p><h2>Wishlist <span>{wishlist.length}</span></h2></div><p className="wishlist-note">Save cards to remember what to trade for next.</p></div><input className="local-search" value={wishlistQuery} onChange={event => setWishlistQuery(event.target.value)} placeholder="Search your wishlist" aria-label="Search your wishlist" />{wishlist.length === 0 ? <div className="empty-state"><span>☆</span><p>Nothing here yet. Tap the star on a search result.</p></div> : visibleWishlist.length === 0 ? <div className="empty-state"><p>No wanted cards match that search.</p></div> : <div className="wishlist-list">{visibleWishlist.map(card => <article key={card.id} className="wishlist-card" draggable onDragStart={event => startDrag(event, card, 'wishlist')} onDragEnd={endDrag}><img src={cardImage(card)} alt="" /><div><strong>{card.name}</strong><span>{card.set_name || card.set?.toUpperCase()} · {card.rarity} · {card.condition || 'Near Mint'}{card.foil ? ' · Foil' : ''}</span></div><div className="quantity"><strong>{card.quantity}</strong></div><button className="edit-button" type="button" onClick={() => openEditor(card, 'wishlist')}>Edit</button><button type="button" onClick={() => toggleWishlist(card)} aria-label={`Remove ${card.name} from wishlist`}>Remove</button></article>)}</div>}</section>
    </main>{editing && <div className="modal-backdrop" role="presentation" onMouseDown={event => event.target === event.currentTarget && setEditing(null)}><form className="edit-modal" onSubmit={saveEdit}><div className="modal-heading"><div><p className="eyebrow">CARD DETAILS</p><h2>Edit {editing.card.name}</h2></div><button className="modal-close" type="button" onClick={() => setEditing(null)} aria-label="Close edit window">×</button></div><div className="edit-fields"><label>Quantity<input type="number" min="1" value={editing.card.quantity} onChange={event => setEditing(prev => ({ ...prev, card: { ...prev.card, quantity: event.target.value } }))} /></label><label>Set<input value={editing.card.set_name} onChange={event => setEditing(prev => ({ ...prev, card: { ...prev.card, set_name: event.target.value } }))} /></label><label>Rarity<select value={editing.card.rarity} onChange={event => setEditing(prev => ({ ...prev, card: { ...prev.card, rarity: event.target.value } }))}><option value="common">Common</option><option value="uncommon">Uncommon</option><option value="rare">Rare</option><option value="mythic">Mythic</option></select></label><label>Condition<select value={editing.card.condition} onChange={event => setEditing(prev => ({ ...prev, card: { ...prev.card, condition: event.target.value } }))}><option>Near Mint</option><option>Lightly Played</option><option>Moderately Played</option><option>Heavily Played</option><option>Damaged</option></select></label><label>Language<input value={editing.card.language} onChange={event => setEditing(prev => ({ ...prev, card: { ...prev.card, language: event.target.value } }))} /></label><label className="checkbox-field"><input type="checkbox" checked={editing.card.foil} onChange={event => setEditing(prev => ({ ...prev, card: { ...prev.card, foil: event.target.checked } }))} /> Foil printing</label><label className="notes-field">Notes<textarea rows="4" value={editing.card.notes} onChange={event => setEditing(prev => ({ ...prev, card: { ...prev.card, notes: event.target.value } }))} placeholder="Where is it stored? What deck needs it?" /></label></div><div className="modal-actions"><button className="cancel-button" type="button" onClick={() => setEditing(null)}>Cancel</button><button className="save-button" type="submit">Save details</button></div></form></div>}<footer>Built for the cards you actually play with.</footer>
  </div>
}
